...
public class A_PersonsController {
    ...
    public ModelAndView workPage() {
        ...
        List<Student> studentsList = studentService.getAll().stream().filter(s -> s.getUser().getId() != null && s.getUser().getId() == getCurrentUser().getId()).collect(Collectors.toList());
//check, if user has this student, add to student list
        //check
        groupsList = groupService.getAll().stream().filter(g -> g.getUser().getId() != null && g.getUser().getId() == getCurrentUser().getId()).collect(Collectors.toList());
...
//        }
    ...
    public String sendMail(HttpServletRequest request, Model model) {
        ...
        for (int i = studenEmail.size() - 1; i >= 0; i--) {
